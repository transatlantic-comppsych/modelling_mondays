...
opts_knit:
    root.dir: "/Users/argyris/modelling_mondays"
editor: visual
---
...
$$f(x | \theta) = \frac{1}{\sqrt{2\pi\sigma^2}} \cdot e^{-\frac{(x - \theta)^2}{2\sigma^2}}$$ {#eq-1}
...
$$L(\mu, \sigma^2 | x_1, x_2, \ldots, x_n) = \prod_{i=1}^{n} \frac{1}{\sqrt{2\pi\sigma^2}} \exp\left(-\frac{(x_i - \mu)^2}{2\sigma^2}\right)$$ {#eq-2}
...
$$y_i = \beta_0 + \beta_1 x_i + \epsilon_i $$ {#eq-5}

where, $\epsilon_i$ follows a normal distribution with mean zero and variance $\sigma^2$

$$L(\beta_0, \beta_1 | x_1, y_1, x_2, y_2, \ldots, x_n, y_n) = \prod_{i=1}^{n} f(y_i | \beta_0 + \beta_1 x_i)$$ {#eq-6}
...
$$f(k;n,p) = \binom{n}{k} p^k (1-p)^{n-k}$$ {#eq-3} where $\binom{n}{k}$ is the binomial coefficient (will explain this) and then come the probabilities.
...
$$ \text{Likelihood  Ratio} = -2*(\text{Log Likelihood}_{0.5} - \text{Log Likelihood}_{ML} )$$ {#eq-4}
...
$$P(A \mid B) \cdot P(B) = P(B \mid A) \cdot P(A)$$ {#eq-10}
...
$$P(homicide\mid man) = \frac{P(man\mid homicide).P(homicide)}{P(man)}$$ {#eq-11}
...
$$P(homicide+|test+) = \frac{P(test+|homicide+).P(homicide+)}{P(test+)}$$ {#eq-12}
...
$$P(homicide+|test+) = \frac{P(test+|homicide+).P(homicide+)}{sensitivity.P(homicide+) + (1-specificity).P(homicide-)}$$ {#eq-13}
...
$$P(gender == female\cap homicide == no) = 50167/10^5 = 0.5017$$ {#eq-14}
...
$$P(gender == female \cap homicide == no) = P(gender == female |homicide == no).P(homicide == no)$$ {#eq-15}
...
$$P(gender == female) = \sum_{\theta} P(\text{female} \mid \text{homiicide}) \cdot P(\text{homicide})$$ {#eq-16}
...
$$P(gender == female \cap homicide == yes) = P(gender == female |homicide == yes).P(homicide == yes)$$ {#eq-17}

Which gives you: (2/20\*20/10\^5) and is the same as doing the following simpler calculation.

$$P(gender == female|homicide == yes) = 2/10^5 = 2x10^-5$$ {#eq-18}
...
$$P(y) = \sum_{\theta} P(\text{y} \mid \theta) \cdot P(\theta)$$ {#eq-19}
...
$$P(y) = \int_{\theta} P(\text{y} \mid \theta) \cdot P(\theta)d\theta$$ {#eq-20}

Then of course the fundamental Bayesian equation is written as:

$$P(\theta\mid y) = \frac {P(y \mid\theta ). P(\theta)}{\sum_{\theta} P(\text{y} \mid \theta) \cdot P(\theta)}$$ {#eq-21}

or, for continuous quantities,

$$P(\theta\mid y) = \frac {P(y \mid\theta ). P(\theta)}{\int_{\theta} P(\text{y} \mid \theta) \cdot P(\theta)d\theta}$$ {#eq-22}
...
$$P(\theta \mid k,n) = beta(\theta|\alpha + k, \beta + n -k) $$ {#eq-23}
...
$$f(x; \alpha, \beta) = \frac{x^{\alpha - 1} (1 - x)^{\beta - 1}}{B(\alpha, \beta)}$$ {#eq-24}
...
$$P(\theta\mid y) \propto P(y \mid\theta ). P(\theta)$$ {#eq-25}
...
$$\hat{\beta}_0 = \overline{y} - \hat{\beta}_1 \overline{x}$$ {#eq-29}
...
# note: filtering for an exact Weight == 90 match is fragile--there may be
# no one at exactly 90kg in this particular sample. Compute the centered
# value directly instead, the same way Weight_centered itself was created.

166.57 + 0.322*(90 - mean(df_hse_wk7$Weight))
```
...
# (compute directly rather than filtering for an exact match on Weight,
# which can silently return an empty vector if no one weighs exactly 90kg
# in this sample--that's what was causing hist() to fail with character(0))
weight_90 <- 90 - mean(df_hse_wk7$Weight)
...
$$exp(\beta_0 + beta_1.x) = \frac{P}{1-P} = odds$$ {#eq-9.8}

This is the connection between the linear equation and the odds

and

$$beta_0 + beta_1.x = ln(\frac{P}{1-P}) = log_odds$$ {#eq-9.9}
...
$$OR = \frac{O(x+1)}{O(x)} = \frac{\frac{P(x+1)}{1-P(x+1)}}{\frac{P(x)}{1-P(x)}}= \frac{exp(\beta_0 + beta_1.(x+1))}{exp(\beta_0 + beta_1.(x))} = exp(\beta_1)$$ {#eq-9.10}
...
list_results_cranach <- list.files("Cranach_results", all.files = T,
...
$$P(i>j) = \frac{P(i)}{P(i) + P(j)}$$ {#eq-11.1}
...
$$logitP(i>j) = log\frac{P(i>j)}{(1-P(i>j))} =  log\frac{P(i>j)}{P_(j>i)} $$ {#eq-11.2}
...
$$logit(P(i>j)) = log\frac{P(i)}{P(j)}= log\frac{e^{\beta_i}}{e^{\beta_j}}= \beta_i - \beta_j$$ If the middle part is difficult to follow, think of the following (which we know to be true because that's how $\log$ works. $$log\frac{P(i)}{P(j)} = log(P(i)) - log(P(j)) = \beta_i - \beta_j$$
