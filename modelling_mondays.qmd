...
prior_beta0 <- function(beta0) {
  dnorm(beta0, mean = 170, sd = 10, log = TRUE)
}

prior_beta1 <- function(beta1) {
...
posterior <- exp(log_posterior - max(log_posterior))  # Subtract max for numerical stability
...
#now you need to translate the vector positions into something that can find the positions 
...
df_to_find_values <- data.frame(table(samples))

row_nums <- as.numeric(as.character(df_to_find_values$samples))
