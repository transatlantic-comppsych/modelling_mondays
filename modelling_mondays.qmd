...
Now imagine that instead of wanting to predict hiegh, you wanted to predic instead whether someone will be "tall" or not. 
We will define "tall" in the HSE dataset to be anything that is above the median height--see code chunk below.
...
$$y_i = \beta_0 + \beta_1.xi + error$$
What is different now is that $y_i$ is no longer a "continuous" quantity, it is binary, in this case 0 or 1, "not tall" vs "tall"
...
Our task is to answer the question: how likely is a person of weight = X to be "tall"?
This is a probability question and probabilities are constrained between 0 and 1 or 0% and 100%. 
The question is then, how do we turn a predictive model into one that uses probabilities? 
...
```
### Exercise 9.1 
Play around with the code chunk above. 
a) vary the slope coefficient.
