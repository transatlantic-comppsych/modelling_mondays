...
The first plot shows the relationship between study_hours and test_scores if we ignore the grouping structure. From this graph it looks like there is no relationship between the two variables. However, when we look at the second plot, we notice that the data is grouped and is showing a linear trend within each school.
...
-   $Y_i$ and $X_i$ represent subject $i$'s outcome (test_scores) and predictor variables (study_hours), respectively.
...
-   $Y_i$ and $X_i$ represent the outcome (test_scores) and predictor variables (study_hours), respectively, for subject $i$ from school $j$.
...
ggplot(data, aes(x = study_hours, y = test_scores, color = school)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE, aes(group = school, color = school)) +
